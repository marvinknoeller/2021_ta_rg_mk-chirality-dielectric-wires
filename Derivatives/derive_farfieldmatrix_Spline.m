function der_ffm = derive_farfieldmatrix_Spline(p,h,Var)
% Input: p is the current parametrization; dimension: 3 x (n+1)
%        h is the direction in which we derive; dimension: 3 x 1
%        Q is the size of matrix, namely Q = 2 * N * (N+2)
%        M is the number of quadrature points to evaluate the integral
%        kappa is the wavenumber
% Output: der_ffm is the derivative of the far field matrix
...
[h_in_between,der_h,~,~] = allpoints(coefsh,tsh,num_x,M);
...
for ki = 1:size(p_in_between,2)
    ...
    der_Pol_2(:,:,ki) = (V_ph*D_eps*(V')+V*D_eps*(V_ph')); %% special case
end
...
integrand4 = HH2;
%%
Help1 = roh^2 * pi * (mu_rel-1) * (integrand1 + integrand2 + integrand3 + integrand4);
...
integrand8 = HH2;
%%
Help2 = -roh^2 * pi * kappa^2 * (1-eps_rel) * (integrand5 + integrand6 + integrand7 + integrand8);
